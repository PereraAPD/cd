...
function setupModernAnimations(section) {
  ...
  if (skillBars.length) {
    skillBars.forEach((bar) => {
      ...
      gsap.fromTo(
        bar.querySelector(".skill-progress"),
        { width: "0%" },
        {
          width: percentage,
          duration: 1.5,
    ...
    submitButton.addEventListener("click", (e) => {
      ...
      gsap.to(ripple, {
        ...
        onComplete: () => {
          ...
          const formContainer = section.querySelector(
  ...
  ScrollTrigger.create({
    ...
    onEnter: () => {
      ...
      if (currentStyle === "old-school" && nextStyle === "modern") {
        ...
        currentStyle === "transitional" &&
        nextStyle === "minimalist"
      ) {
 ...
 * @param {HTMLElement} transitionalSection - The transitional section
 */
function reverseTransitionTransitionalToModern(
  modernSection,
  transitionalSection
) {
  // Create overlay for transition
 ...
 */
function transitionTransitionalToMinimalist(
  transitionalSection,
  minimalistSection
) {
  // Create overlay for transition
  ...
  const elements = document.querySelectorAll(
    ".card, .about-card, .project-card, button, input, textarea"
  );
  if (elements.length) {
    gsap.to(elements, {
 ...
 * @param {HTMLElement} minimalistSection - The minimalist section
 */
function reverseTransitionMinimalistToTransitional(
  transitionalSection,
  minimalistSection
) {
  // Create overlay for transition
...
function setupElementAnimations() {
  ...
  // Fade in with delay
  const fadeDelayElements = document.querySelectorAll(
    '[data-scroll-element="fade-in-delay"]'
  );
  ...
  // Scale in with delay
  const scaleDelayElements = document.querySelectorAll(
    '[data-scroll-element="scale-in-delay"]'
  );
  ...
  const staggerElements = document.querySelectorAll(
    '[data-scroll-element="stagger-fade"]'
  );
...
// Leaves Animation with Wind Effect
...
function createLeaf(container) {
  ...
  leaf.classList.add(`shape-${shapeNum}`);

  // Random color (1-8)
  const colorNum = Math.floor(Math.random() * 8) + 1;
  leaf.classList.add(`color-${colorNum}`);

  // Randomize leaf properties
  const startPositionX = Math.random() * 100; // Random position across the width
  const fallDuration = 10 + Math.random() * 10; // Faster falling (10-20 seconds)
  const fallDelay = Math.random() * -15; // Shorter delay spread
  ...
  leaf.style.setProperty("--start-x", `${startPositionX}%`);
  leaf.style.width = `${size}px`;
  leaf.style.height = `${size}px`;

  container.appendChild(leaf);

  // Remove and recreate leaf when animation ends
...
function createLeaves() {
  ...
  sections.forEach((section) => {
    ...
    section.appendChild(leavesContainer);

    // Create more leaves with staggered timing
    for (let i = 0; i < 50; i++) {
      setTimeout(() => createLeaf(leavesContainer), Math.random() * 2000); // Random start times within first 2 seconds
    }
  });

  // Start wind simulation
  simulateWind(windIndicator);
}

function simulateWind(windIndicator) {
  const updateWind = () => {
    // Random wind direction and intensity
    const newWindIntensity = Math.random() > 0.5 ? 2 : -2; // Strong wind in one direction

    // Update all leaves with new wind intensity
    document.querySelectorAll(".leaf").forEach((leaf) => {
      leaf.style.setProperty("--wind-intensity", newWindIntensity);
    });

    // Update wind effect
    windEffect.classList.remove("left", "right", "active");
    void windEffect.offsetWidth; // Force reflow
    windEffect.classList.add(newWindIntensity > 0 ? "right" : "left", "active");

    // Update wind indicator
    currentWindIntensity = newWindIntensity;
    windIndicator.textContent = `WIND ${
      newWindIntensity > 0 ? "RIGHT" : "LEFT"
    }`;
    windIndicator.classList.add("active");

    // Schedule wind effect removal
    setTimeout(() => {
      windEffect.classList.remove("active");
      windIndicator.classList.remove("active");
    }, 5000); // Wind effect lasts 5 seconds

    // Schedule next wind update (30 seconds)
    setTimeout(updateWind, 30000);
  };
